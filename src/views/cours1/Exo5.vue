<template>
  <div>
    ...
    <h3> Exercice 5 : </h3>
    <p> Rajoutez une deuxième bouton à id-card qui permet d’éditer les champs. </p>
    ...
    <card-list
      :data="items"
      @deleteItem="(user) => removeItem(user)"
      @editItem="(user) => editItem(user)"
    />
  </div>
</template>

<script>
import CardList from '@/components/cours1/exo5/CardList'
...
export default {
  ...
  methods: {
    ...
    removeItem (user) {
      this.items = this.items.filter((el) => el.lastName !== user.lastName)
    },
    editItem (user) {
      this.items.forEach((el) => {
        if (el.lastName === user.lastName) {
